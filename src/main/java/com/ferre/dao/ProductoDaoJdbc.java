...
public class ProductoDaoJdbc implements ProductoDao {
    ...
    @Override public void crear(Producto p) {
        ...
        try (Connection cn = DataSourceFactory.getConnection(); PreparedStatement ps = cn.prepareStatement(sql)){
            ps.setString(1,p.getCodigo()); ps.setString(2,p.getNombre()); ps.setString(3,p.getUnidad());
            ps.setBigDecimal(4,p.getPrecio()); ps.setInt(5,p.getStock());
            if (p.getProveedorId()==null) ps.setNull(6, Types.BIGINT); else ps.setLong(6,p.getProveedorId());
    ...
    @Override public void cambiarEstado(long id, boolean activo) {
        ...
        try (Connection cn = DataSourceFactory.getConnection(); PreparedStatement ps = cn.prepareStatement(sql)){
            ps.setInt(1, activo?1:0); ps.setLong(2,id); ps.executeUpdate();
        } catch(Exception e){ throw new RuntimeException(e); }
    }

    @Override public Optional<Producto> findByCodigo(String codigo) {
