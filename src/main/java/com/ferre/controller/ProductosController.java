...
public class ProductosController {
    ...
    @FXML private TableColumn<Producto,String> colId,colCodigo,colNombre,colUnidad,colPrecio,colStock;
    @FXML private TableColumn<Producto,Boolean> colActivo;
    @FXML private TextField txtBuscar, txtCodigo, txtNombre, txtUnidad, txtPrecio, txtStock;
    ...
    public void initialize(){
        ...
        colActivo.setCellFactory(col -> new TableCell<>(){
            @Override protected void updateItem(Boolean b, boolean empty){
                super.updateItem(b,empty); setText(empty?null:(b?"Sí":"No"));
            }
        });
        ...
        tbl.getSelectionModel().selectedItemProperty().addListener((o,old,now)->{
            ...
            if (now!=null){
                ...
                if (now.getProveedorId()==null) cmbProveedor.getSelectionModel().clearSelection();
                else cmbProveedor.getSelectionModel().select(
    ...
    @FXML private void nuevo(){
        seleccionado = null;
        txtCodigo.clear(); txtNombre.clear(); txtUnidad.clear();
    ...
    @FXML private void guardar(){
        try {
            ...
            p.setCodigo(codigo); p.setNombre(nombre); p.setUnidad(unidad);
            p.setPrecio(precio); p.setStock(stock); p.setProveedorId(provId);
            if (p.getId()==0) p.setActivo(true);
    ...
    @FXML private void toggleEstado(){
        if (seleccionado==null){ warn("Seleccione un producto"); return; }
        try {
    ...
    private void alert(Alert.AlertType t, String h, String m){ var a=new Alert(t); a.setHeaderText(h); a.setContentText(m); a.setTitle("Productos"); a.showAndWait(); }
}
