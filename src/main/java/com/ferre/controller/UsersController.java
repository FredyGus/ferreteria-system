...
public class UsersController {
    ...
    @FXML private TableColumn<Usuario,String> colId, colUsuario, colNombre, colRol;
    @FXML private TableColumn<Usuario,Boolean> colEstado;
    @FXML private TextField txtUsuario, txtNombre;
    ...
    public void initialize(){
        ...
        colEstado.setCellFactory(col -> new TableCell<>() {
            @Override protected void updateItem(Boolean ok, boolean empty) {
    ...
    @FXML private void nuevo(){
        seleccionado = null;
        txtUsuario.clear(); txtNombre.clear(); txtPassword.clear(); cmbRol.setValue(Rol.VENTAS);
    ...
    @FXML private void guardar(ActionEvent e){
        ...
        if (u.isEmpty() || n.isEmpty() || rol==null){
            alert(Alert.AlertType.WARNING,"Datos incompletos","Usuario, nombre y rol son requeridos"); return;
        }
        try{
            if (seleccionado == null){
                String pwd = txtPassword.getText();
                if (pwd==null || pwd.isBlank()){
                    alert(Alert.AlertType.WARNING,"Contraseña requerida","Para crear usuario ingresa contraseña"); return;
                }
    ...
    @FXML private void resetPassword(){
        if (seleccionado==null){ alert(Alert.AlertType.WARNING,"Seleccione","Selecciona un usuario"); return; }
        String pwd = txtPassword.getText();
        if (pwd==null || pwd.isBlank()){ alert(Alert.AlertType.WARNING,"Contraseña","Ingresa nueva contraseña"); return; }
        try{
    ...
    private void alert(Alert.AlertType t, String h, String m){
        var a = new Alert(t); a.setHeaderText(h); a.setContentText(m); a.setTitle("Usuarios"); a.showAndWait();
    }
}
